...
- Follow-up: first GitHub Actions package-manager build reached the source build but failed before compile because the runner did not have the absolute Aqua target prefix `/data/data/com.andropy.ide` available for host-side staging; restored host staging setup for only Aqua's package id, not Termux paths.
- Next: push the staging fix, trigger fresh `termux-core dpkg apt` package builds for emulator and device arches, then install the rebuilt packages into the app prefix and re-test `apt install`.
